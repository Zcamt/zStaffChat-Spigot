...
public class BungeeToggleDevChatCommand extends BaseCommand {
    ...
    public BungeeToggleDevChatCommand(BungeeChatManager bungeeChatManager) {
    ...
    public void onCommand(ProxiedPlayer player, String[] args){
        ...
        if(args.length != 0){
            BungeeUtilities.sendMessage(player, BungeeConfigManager.devchatPrefix + "&r &7Usage: /dctoggle");
            return;
        }
        if(disabledChatsManager.contains(player.getUniqueId(), chatType)){
            BungeeUtilities.sendMessage(player, BungeeConfigManager.devchatPrefix + "&r &cError: Cannot toggle a chat you've disabled");
            return;
        }
